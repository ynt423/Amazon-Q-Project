...
"""
        ...
        return f"""
請為 {ticker} 股票提供一份全面的投資分析報告，嚴格按照以下12點結構格式：
...
**請嚴格按照以下格式，每個標題佔一行，分析內容從新行開始：**

1. 開場總結 (技術分析 50%)：
[基於技術指標的整體評估，包含綜合評分和RS評級]

2. 技術分析 (技術分析 50%)：
[RSI、MACD等指標的具體數值和含義分析]

3. 形態分析 (技術分析 50%)：
[形態識別結果的意義和突破概率分析]

4. 新聞影響 (新聞分析 20%)：
[引用具體新聞標題和對股價的影響，使用HTML連結格式]
...
**格式要求：**
- 每個標題必須單獨一行，格式為「數字. 標題 (權重類別)：」
- 分析內容從新行開始，不要與標題在同一行
- 使用專業但易懂的語言，不要使用任何emoji表情符號
- 在適當的地方引用新聞來源，使用HTML格式：<a href="URL" target="_blank" rel="noopener noreferrer">新聞標題</a>
- 每段分析都要有具體的數據支撐
- 保持專業的投資分析報告風格
"""
